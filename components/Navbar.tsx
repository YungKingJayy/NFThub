    ...
    <>
      ...
      <nav className={clsx("block md:hidden w-screen h-screen bg-black absolute transition-all ease-in-out", {
        ...
        <div className="flex items-center justify-between pl-[1.62rem] pr-[1.1rem] py-9 bottom-border">
          ...
          <button onClick={()=>{toggle()}}><Image src="/icons/close.svg" alt="logo" width={30} height={1} /></button>
        </div>
        <ul className="flex flex-col">
          <li className="nav-link-mobile flex items-center justify-between">
            <Link href="#">Home</Link>
            <button className="flex items-center justify-center bg-white w-8 h-8 rounded-md"><Image src="/icons/add.svg" alt="logo" width={12} height={12} /></button>
          </li>
